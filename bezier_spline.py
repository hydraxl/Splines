...
class Bezier_Spline(Spline):
    ...
    def sample(self, n=10):
        ...
        if (num_controls - 1) % 3 != 0:
            raise ValueError(f"Bezier Spline has {len(self.control_points)} control points, which is not one more than a multiple of 3")
        
        # Setup variables
        num_sections = (num_controls - 1) / 3
        line_length = (n * num_sections) + 1
        line = np.empty((line_length, 2))
        
        # Sample the spline
        for s in range(num_sections):
            section = self.control_points[s*3 : s*3 + 4]
            
            # Use linear interpolation to sample each bezier curve within the spline
            for i in range(n):

                # Create initial set of temporary points
                temp_points = np.empty((3, 2))
                for p in range(3):
